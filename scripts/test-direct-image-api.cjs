...
async function run(model,mergeMode,aspectRatio){
 ...
 if(model.startsWith('gpt-image')){
  const {request,url}=http.at(-1),form=request.body;assert.equal(url,'https://api.openai.com/v1/images/edits');assert.equal(form.get('prompt'),req.body.prompt.trim());assert.equal(form.get('model'),'gpt-image-2');assert.equal(form.get('input_fidelity'),null);assert.equal(form.get('size'),'1152x1536');
...
(async()=>{editorSettings();for(const mode of ['custom','combine','replace_background','replace_product','replace_person','add_logo'])await run('gemini-3.1-flash-image',mode,'3:4');await run('gemini-3.1-flash-image','custom','prompt');await run('gemini-3.1-flash-image','combine','auto');await run('gpt-image-2:high','combine','3:4');assert.equal(calls.length,8,'exactly one Gemini call per image, no describe/regenerate intermediate');console.log('PASS direct-image API originals/roles/order/MIME, exact prompts, native aspect parameters, GPT multipart and first-page settings migration');})().catch(error=>{console.error(error);process.exitCode=1});
