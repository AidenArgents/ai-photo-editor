...
如果窗口提示更新失败，请先确认网络/系统代理可用（浏览器能打开 GitHub 即可），然后重新双击 `update.bat`。本地现有版本不受影响，可继续使用。
...
如果最初是直接解压 ZIP，先在项目文件夹双击 `setup.bat`。它会把当前文件夹准备成可更新的 Git 安装；完成后再双击 `update.bat`。
