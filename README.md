...
更新时 GitHub 上的源代码版本优先。即使本地修改过源代码，更新工具也会先把本地改动保存到 Git stash，再应用 GitHub 最新版本；本地提交会保存到 `backup-before-update-*` 分支。`.runtime`、`node_modules`、日志、环境配置和浏览器中的 API Key 不会被删除。

如果窗口提示更新失败，请查看前面的具体错误。更新工具会显示失败阶段；常见情况是 GitHub 网络/权限不可用，或 3000 端口仍被其他程序占用。更新失败时，已创建的本地备份会保留。
